...
# -*- coding: utf-8 -*-
"""
Descarga la oferta gastronomica (cuisine=* de OpenStreetMap y los lugares
que sirven comida) del distrito de SAN JUAN OPICO, El Salvador.

La llave cuisine=* describe el tipo de comida que sirve un lugar, asi que
este archivo trae tanto los elementos con cuisine=* como los locales de
comida que aun no la tienen: restaurantes, comida rapida, pupuserias,
comedores, cafeterias, bares, pizzerias, taquerias, marisquerias,
churrasquerias, heladerias, panaderias, pastelerias y tiendas de comida.
...
Uso:   pip install requests
       python3 descargar_cuisine_san_juan_opico.py

Deja en la carpeta 'datos':
    gastronomia_san_juan_opico.csv
    gastronomia_san_juan_opico.geojson

Los CSV de todas las categorias y distritos tienen las mismas columnas, asi
que al final se pueden unir con:
    head -1 datos/gastronomia_san_juan_opico.csv >  datos/todos.csv
...
# Filtros de la categoria cuisine=*. Primero se pide la llave completa, asi
...
FILTROS_POR_TAG = [
    'nwr["cuisine"]',
    'nwr["amenity"~"^(restaurant|fast_food|cafe|bar|pub|biergarten|ice_cream|food_court|canteen|nightclub|marketplace)$"]',
    'nwr["shop"~"^(bakery|pastry|deli|butcher|greengrocer|confectionery|coffee|tea|seafood|cheese|chocolate|beverages|alcohol|wine|food|dairy|farm|frozen_food|health_food|spices|tortilla)$"]',
    'nwr["craft"~"^(bakery|brewery|distillery|winery|caterer|confectionery)$"]',
]
...
FILTROS_POR_NOMBRE = [
    'nwr["name"~"restaurante|restaurant|comedor|pupuser|antojitos|t[íi]picos|cafeter[íi]a|caf[ée] |coffee|panader|pasteler|reposter|helader|sorbeter|pizzer|pizza|taquer|tacos|marisquer|mariscos|cevicher|churrasquer|parrilla|asados|carnes|buffet|food|grill|bar |cervecer|licorer|rosticer|pollo|cocina|kitchen|sushi|wok|china|burger|sandwich|tortiller|pupusas|yogurt|smoothie|juguer",i][!"highway"][!"landuse"][!"place"][!"boundary"]',
]
...
# categoria manda cuisine; las demas solo entran si el elemento no la trae.
LLAVES_TIPO = ["cuisine", "shop", "amenity", "healthcare", "office",
