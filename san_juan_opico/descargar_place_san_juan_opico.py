...
# -*- coding: utf-8 -*-
"""
Descarga los lugares poblados (place=* de OpenStreetMap) del distrito de SAN
JUAN OPICO, El Salvador.

Incluye barrios, colonias, residenciales y reparto, comunidades, cantones y
caserios, urbanizaciones y lotificaciones, y los limites del propio
distrito. Como en El Salvador muchas colonias estan mapeadas como
landuse=residential con nombre en vez de place=*, tambien se piden esas.
...
Uso:   pip install requests
       python3 descargar_place_san_juan_opico.py

Deja en la carpeta 'datos':
    lugares_san_juan_opico.csv
    lugares_san_juan_opico.geojson

Los CSV de todas las categorias y distritos tienen las mismas columnas, asi
que al final se pueden unir con:
    head -1 datos/lugares_san_juan_opico.csv >  datos/todos.csv
...
# Filtros de la categoria place=*. Primero se pide la llave completa, asi
...
FILTROS_POR_TAG = [
    ...
    'nwr["boundary"="administrative"]["admin_level"~"^(8|9|10)$"]',
    'nwr["addr:suburb"]["place"]',
]
...
FILTROS_POR_NOMBRE = [
    'nwr["name"~"colonia|reparto|residencial|urbanizaci[óo]n|lotificaci[óo]n|comunidad|cant[óo]n|caser[íi]o|barrio|finca|hacienda|villa|ciudadela|jardines de|bosques de|altos de|valle de|lomas de|prados de|brisas|monta[ñn]as de|ciudad ",i][!"highway"][!"boundary"]',
]
...
# categoria manda place; las demas solo entran si el elemento no la trae.
LLAVES_TIPO = ["place", "shop", "amenity", "healthcare", "office",
               "craft", "industrial", "tourism", "leisure", "sport",
               "club", "historic", "man_made", "building", "landuse",
               "natural", "cuisine", "brand"]
