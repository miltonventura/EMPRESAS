...
# -*- coding: utf-8 -*-
"""
Descarga la industria (industrial=* de OpenStreetMap y sus llaves vecinas)
del distrito de SAN JUAN OPICO, El Salvador.

Incluye fabricas y plantas de produccion, maquilas y zonas francas,
procesadoras y empacadoras de alimentos, embotelladoras, molinos, talleres
industriales, bodegas y almacenadoras, plantas de tratamiento, subestaciones
y generacion electrica, tanques y silos, canteras y suelo de uso industrial.
...
Uso:   pip install requests
       python3 descargar_industrial_san_juan_opico.py

Deja en la carpeta 'datos':
    industrias_san_juan_opico.csv
    industrias_san_juan_opico.geojson

Los CSV de todas las categorias y distritos tienen las mismas columnas, asi
que al final se pueden unir con:
    head -1 datos/industrias_san_juan_opico.csv >  datos/todos.csv
...
# Filtros de la categoria industrial=*. Primero se pide la llave completa, asi
...
FILTROS_POR_TAG = [
    'nwr["industrial"]',
    'nwr["landuse"~"^(industrial|quarry|port|depot|logistics)$"]',
    'nwr["man_made"~"^(works|wastewater_plant|water_works|pumping_station|storage_tank|silo|kiln|gasometer|chimney|petroleum_well|mineshaft|adit|crane)$"]',
    'nwr["building"~"^(industrial|warehouse|factory|manufacture|hangar)$"]',
...
FILTROS_POR_NOMBRE = [
    'nwr["name"~"industri|f[áa]brica|fabrica|manufactur|maquila|planta |procesadora|empacadora|beneficio|ingenio|embotellador|envasadora|molino|aserradero|bodega|almacenadora|zona franca|parque industrial|metalmec|metal[úu]rgic|fundici[óo]n|pl[áa]stico|qu[íi]mic|textil|hilander|cementera|concretera|ladrillera|bloquera|curtiembre|refiner|destiler|cervecer|l[áa]cteos|avicola|av[íi]cola|granja|distribuidora",i][!"highway"][!"place"][!"boundary"]',
]
...
# categoria manda industrial; las demas solo entran si el elemento no la trae.
LLAVES_TIPO = ["industrial", "shop", "amenity", "healthcare", "office",
               "craft", "tourism", "leisure", "sport", "club", "historic",
               "man_made", "building", "landuse", "natural", "place",
               "cuisine", "brand"]
