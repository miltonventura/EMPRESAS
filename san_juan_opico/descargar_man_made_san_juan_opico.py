...
# -*- coding: utf-8 -*-
"""
Descarga la infraestructura construida (man_made=* de OpenStreetMap) del
distrito de SAN JUAN OPICO, El Salvador.

Incluye torres y antenas de telecomunicaciones, tanques y cisternas, pozos,
plantas de tratamiento y de bombeo, subestaciones y torres electricas,
puentes y muelles, chimeneas, silos, faros, miradores construidos,
acueductos, represas, postes de servicio y demas obras de man_made=*.
...
Uso:   pip install requests
       python3 descargar_man_made_san_juan_opico.py

Deja en la carpeta 'datos':
    infraestructura_san_juan_opico.csv
    infraestructura_san_juan_opico.geojson

Los CSV de todas las categorias y distritos tienen las mismas columnas, asi
que al final se pueden unir con:
    head -1 datos/infraestructura_san_juan_opico.csv >  datos/todos.csv
...
# Filtros de la categoria man_made=*. Primero se pide la llave completa, asi
...
FILTROS_POR_TAG = [
    'nwr["man_made"]',
    'nwr["power"~"^(plant|substation|generator|tower|transformer|portal)$"]',
    'nwr["tower:type"]',
    'nwr["telecom"]',
    'nwr["building"~"^(water_tower|storage_tank|transformer_tower|service|bridge)$"]',
    'nwr["bridge"]["name"]',
]
...
FILTROS_POR_NOMBRE = [
    'nwr["name"~"torre|antena|repetidora|tanque|cisterna|pozo|planta de|subestaci[óo]n|puente|paso a desnivel|muelle|chimenea|silo|faro|mirador|acueducto|represa|presa|bomba|rebombeo|anda |del sur|el[ée]ctrica|transmisi[óo]n",i][!"highway"][!"landuse"][!"place"][!"boundary"]',
]
...
# categoria manda man_made; las demas solo entran si el elemento no la trae.
LLAVES_TIPO = ["man_made", "shop", "amenity", "healthcare", "office",
               "craft", "industrial", "tourism", "leisure", "sport",
               "club", "historic", "building", "landuse", "natural",
               "place", "cuisine", "brand"]
