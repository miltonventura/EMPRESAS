...
# -*- coding: utf-8 -*-
"""
Descarga los usos de suelo (landuse=* de OpenStreetMap) del distrito de SAN
JUAN OPICO, El Salvador.

Incluye suelo residencial, comercial, industrial y de retail, colonias y
lotificaciones, zonas francas y parques industriales, areas institucionales
y educativas, cementerios, areas verdes y recreativas, bosques y cultivos,
pastizales, canteras y cuerpos de agua. Son casi todos poligonos: el CSV
guarda el centro de cada uno.
...
Uso:   pip install requests
       python3 descargar_landuse_san_juan_opico.py

Deja en la carpeta 'datos':
    usos_de_suelo_san_juan_opico.csv
    usos_de_suelo_san_juan_opico.geojson

Los CSV de todas las categorias y distritos tienen las mismas columnas, asi
que al final se pueden unir con:
    head -1 datos/usos_de_suelo_san_juan_opico.csv >  datos/todos.csv
...
# Filtros de la categoria landuse=*. Primero se pide la llave completa, asi
...
FILTROS_POR_TAG = [
    'nwr["landuse"]',
    'nwr["leisure"~"^(park|garden|recreation_ground|pitch|golf_course|nature_reserve)$"]',
    'nwr["natural"~"^(wood|scrub|grassland|heath|wetland|water|sand|bare_rock)$"]',
    'nwr["amenity"~"^(grave_yard|marketplace|parking)$"]["name"]',
...
FILTROS_POR_NOMBRE = [
    'nwr["name"~"finca|hacienda|cafetal|ca[ñn]al|parcela|lotificaci[óo]n|urbanizaci[óo]n|zona franca|parque industrial|residencial|cementerio|camposanto|vivero|granja|plantaci[óo]n|potrero|terreno|predio|reserva",i][!"highway"][!"place"][!"boundary"]',
]

CATEGORIA = "usos_de_suelo"
...
# categoria manda landuse; las demas solo entran si el elemento no la trae.
LLAVES_TIPO = ["landuse", "shop", "amenity", "healthcare", "office",
               "craft", "industrial", "tourism", "leisure", "sport",
               "club", "historic", "man_made", "building", "natural",
               "place", "cuisine", "brand"]
