...
# -*- coding: utf-8 -*-
"""
Descarga los elementos naturales (natural=* de OpenStreetMap) del distrito
de SAN JUAN OPICO, El Salvador.

Incluye cerros, volcanes y lomas, arboles y arboledas, bosques y matorrales,
rios, quebradas y nacimientos de agua, lagunas, manantiales, cuevas,
barrancos y acantilados, humedales, pastizales y suelo desnudo. Se agregan
las areas protegidas y los cuerpos de agua mapeados con otras llaves.
...
Uso:   pip install requests
       python3 descargar_natural_san_juan_opico.py

Deja en la carpeta 'datos':
    naturaleza_san_juan_opico.csv
    naturaleza_san_juan_opico.geojson

Los CSV de todas las categorias y distritos tienen las mismas columnas, asi
que al final se pueden unir con:
    head -1 datos/naturaleza_san_juan_opico.csv >  datos/todos.csv
...
# Filtros de la categoria natural=*. Primero se pide la llave completa, asi
...
FILTROS_POR_TAG = [
    'nwr["natural"]',
    'nwr["waterway"~"^(river|stream|waterfall|riverbank|spring|canal|ditch)$"]',
    'nwr["landuse"~"^(forest|meadow|orchard|vineyard|grass|greenfield|farmland)$"]',
    'nwr["leisure"~"^(nature_reserve|park|garden)$"]',
...
FILTROS_POR_NOMBRE = [
    'nwr["name"~"cerro|volc[áa]n|loma|monta[ñn]a|cordillera|r[íi]o |quebrada|riachuelo|arenal|laguna|lago |playa|bosque|arboleda|parque nacional|reserva|manglar|cascada|salto |ca[ñn][óo]n|barranc|cueva|nacimiento|ojo de agua|manantial|pe[ñn][óo]n|roca|isla",i][!"highway"][!"place"][!"boundary"]',
]
...
# categoria manda natural; las demas solo entran si el elemento no la trae.
LLAVES_TIPO = ["natural", "shop", "amenity", "healthcare", "office",
