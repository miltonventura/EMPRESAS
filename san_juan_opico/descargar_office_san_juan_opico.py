...
# -*- coding: utf-8 -*-
"""
Descarga las oficinas y empresas de servicios (office=* de OpenStreetMap)
del distrito de SAN JUAN OPICO, El Salvador.

Incluye bufetes de abogados, notarias, contadores y auditores, consultorias,
inmobiliarias, aseguradoras y corredurias, financieras, cooperativas, bancos
y sus agencias, remesadoras, empresas de publicidad y mercadeo, arquitectura
e ingenieria, constructoras, logistica y agencias aduanales, courier, call
centers, agencias de empleo, agencias de viajes, telecomunicaciones,
empresas de informatica, ONG y oficinas de gobierno, entre el resto de los
valores de office=*.
...
Uso:   pip install requests
       python3 descargar_office_san_juan_opico.py

Deja en la carpeta 'datos':
    oficinas_san_juan_opico.csv
    oficinas_san_juan_opico.geojson

Los CSV de todas las categorias y distritos tienen las mismas columnas, asi
que al final se pueden unir con:
    head -1 datos/oficinas_san_juan_opico.csv >  datos/todos.csv
    tail -q -n +2 datos/*_san_juan_opico.csv       >> datos/todos.csv
...
# Filtros de la categoria office=*. Primero se pide la llave completa, asi
...
FILTROS_POR_TAG = [
    'nwr["office"]',
    'nwr["amenity"~"^(bank|bureau_de_change|money_transfer|money_lender|post_office|courthouse|townhall|embassy|public_building|conference_centre|coworking_space|lawyer|accountant|insurance|employment_agency)$"]',
    'nwr["shop"~"^(estate_agent|insurance|travel_agency|employment_agency|financial_advisor|money_lender|copyshop|it|telecommunication)$"]',
    'nwr["landuse"="commercial"]["name"]',
    'nwr["building"~"^(office|commercial|government|civic|public)$"]["name"]',
]
...
FILTROS_POR_NOMBRE = [
    'nwr["name"~"oficin|bufete|abogad|notar|jur[ií]dic|contador|contabil|auditor|consultor|asesor|inmobiliar|bienes ra[ií]ces|corredur|seguros|aseguradora|afp |financier|cr[ée]dito|cooperativa|caja de cr[ée]dito|banco |sucursal|agencia|remesas|casa de cambio|publicidad|mercadeo|arquitect|ingenier|constructora|logistic|aduanal|courier|encomiendas|call center|outsourcing|recursos humanos|bolsa de empleo|agencia de viajes|imprenta|editorial|telecomunicaciones|inform[áa]tic|software|sistemas|tecnolog|corporaci[óo]n|ministerio|viceministerio|direcci[óo]n general|instituto nacional",i][!"highway"][!"landuse"][!"place"][!"boundary"]',
]
...
# categoria manda office; las demas solo entran si el elemento no la trae.
LLAVES_TIPO = ["office", "shop", "amenity", "healthcare", "craft",
...
BBOX_BUSQUEDA = "13.65,-89.65,14.20,-89.10"
