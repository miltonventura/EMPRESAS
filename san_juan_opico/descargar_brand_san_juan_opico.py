...
# -*- coding: utf-8 -*-
"""
Descarga los lugares con marca o cadena comercial identificada (brand=* de
OpenStreetMap) del distrito de SAN JUAN OPICO, El Salvador.

Sirve para medir la presencia de franquicias y cadenas: supermercados,
gasolineras, bancos, restaurantes de comida rapida, farmacias, tiendas de
telefonia y demas negocios que en OpenStreetMap llevan brand=* o
brand:wikidata=*. El filtro por nombre agrega las cadenas mas conocidas de
El Salvador para recuperar las sucursales que quedaron sin la llave brand.
...
Uso:   pip install requests
       python3 descargar_brand_san_juan_opico.py

Deja en la carpeta 'datos':
    marcas_san_juan_opico.csv
    marcas_san_juan_opico.geojson

Los CSV de todas las categorias y distritos tienen las mismas columnas, asi
que al final se pueden unir con:
    head -1 datos/marcas_san_juan_opico.csv >  datos/todos.csv
...
# Filtros de la categoria brand=*. Primero se pide la llave completa, asi
...
FILTROS_POR_TAG = [
    'nwr["brand"]',
    'nwr["brand:wikidata"]',
    'nwr["operator"]["operator:type"~"^(private|public|company)$"]',
]
...
FILTROS_POR_NOMBRE = [
    'nwr["name"~"pollo campero|super selectos|despensa de don juan|la despensa|walmart|maxi despensa|pricesmart|super repuestos|texaco|puma|uno |alba |petro|banco agr[ií]cola|banco cuscatl[áa]n|davivienda|scotiabank|promerica|banco hipotecario|banco de fomento|bancoagr[ií]cola|citi|claro|tigo|movistar|digicel|pizza hut|domino|papa john|burger king|mcdonald|wendy|subway|starbucks|the coffee cup|biggest|china wok|el sopon|dollar city|farmacia san nicol[áa]s|farmacias econ[óo]micas|farmacia uno|siman|omnisport|la curacao|curacao|almacenes tropigas|prado|freund|vidr[ií]|epa |ace |office depot|cin[ée]polis|multicinemas|metrocentro|galer[ií]as|plaza mundo|las cascadas",i][!"highway"][!"landuse"][!"place"][!"boundary"]',
]
...
# categoria manda brand; las demas solo entran si el elemento no la trae.
LLAVES_TIPO = ["brand", "shop", "amenity", "healthcare", "office",
               "craft", "industrial", "tourism", "leisure", "sport",
               "club", "historic", "man_made", "building", "landuse",
               "natural", "place", "cuisine"]
