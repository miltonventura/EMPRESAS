...
# -*- coding: utf-8 -*-
"""
Descarga los lugares turisticos y de hospedaje (tourism=* de OpenStreetMap)
del distrito de SAN JUAN OPICO, El Salvador.

Incluye hoteles, hostales, moteles, apartahoteles, posadas y cabanas,
museos, galerias de arte, atracciones y puntos de interes, miradores,
informacion turistica, campamentos, obras de arte urbano y sitios
historicos con interes turistico.
...
Uso:   pip install requests
       python3 descargar_tourism_san_juan_opico.py

Deja en la carpeta 'datos':
    turismo_san_juan_opico.csv
    turismo_san_juan_opico.geojson

Los CSV de todas las categorias y distritos tienen las mismas columnas, asi
que al final se pueden unir con:
    head -1 datos/turismo_san_juan_opico.csv >  datos/todos.csv
...
# Filtros de la categoria tourism=*. Primero se pide la llave completa, asi
...
FILTROS_POR_TAG = [
    'nwr["tourism"]',
    'nwr["historic"]',
    'nwr["building"~"^(hotel|hostel|motel|museum)$"]',
    'nwr["leisure"~"^(resort|beach_resort|water_park)$"]',
    'nwr["shop"="travel_agency"]',
    'nwr["office"="guide"]',
]
...
FILTROS_POR_NOMBRE = [
    'nwr["name"~"hotel|hostal|motel|posada|hospedaje|apartahotel|caba[ñn]a|albergue|resort|museo|galer[íi]a de arte|casa de la cultura|mirador|turicentro|parque nacional|tour|turismo|agencia de viajes|monumento|sitio arqueol|centro de visitantes|informaci[óo]n tur",i][!"highway"][!"landuse"][!"place"][!"boundary"]',
]
...
# categoria manda tourism; las demas solo entran si el elemento no la trae.
LLAVES_TIPO = ["tourism", "shop", "amenity", "healthcare", "office",
               "craft", "industrial", "leisure", "sport", "club",
