...
# -*- coding: utf-8 -*-
"""
Descarga los talleres y oficios (craft=* de OpenStreetMap) del distrito de
SAN JUAN OPICO, El Salvador.

Incluye talleres de mecanica y enderezado y pintura, herrerias, carpinterias
y ebanisterias, soldadura, hojalaterias, electricistas, fontaneros,
cerrajerias, tapicerias, sastrerias y modisterias, zapaterias de reparacion,
joyeros y orfebres, relojeros, vidrierias, rotulacion y serigrafia,
imprentas, panaderias artesanales, artesanos, ceramistas y el resto de los
valores de craft=*.
...
Uso:   pip install requests
       python3 descargar_craft_san_juan_opico.py

Deja en la carpeta 'datos':
    oficios_san_juan_opico.csv
    oficios_san_juan_opico.geojson

Los CSV de todas las categorias y distritos tienen las mismas columnas, asi
que al final se pueden unir con:
    head -1 datos/oficios_san_juan_opico.csv >  datos/todos.csv
...
# Filtros de la categoria craft=*. Primero se pide la llave completa, asi
...
FILTROS_POR_TAG = [
    'nwr["craft"]',
    'nwr["shop"~"^(car_repair|motorcycle_repair|bicycle|shoe_repair|tailor|dressmaker|sewing|locksmith|electronics_repair|mobile_phone_repair|computer_repair|repair|watchmaker|goldsmith|jewelry_repair|upholsterer|carpenter|glaziery|signs|printing|trade|hardware|paint|frame|pottery|craft)$"]',
    'nwr["amenity"~"^(car_repair|workshop|vehicle_inspection)$"]',
    'nwr["building"~"^(workshop|industrial|service)$"]["name"]',
]
...
FILTROS_POR_NOMBRE = [
    'nwr["name"~"taller|mec[áa]nic|enderezado|pintura automotriz|electroaut|herrer|forja|carpinter|ebanister|madera|soldadur|hojalater|torno|electricista|fontaner|plomer|refrigeraci[óo]n|aire acondicionado|alba[ñn]il|cerrajer|tapicer|sastre|costur|modister|bordad|zapater|reparaci[óo]n|joyer|orfebr|relojer|vidrier|aluminio|r[óo]tulos|serigraf|imprenta|litograf|grabado|artesan|cer[áa]mic|alfarer|a[ñn]il|hamaca|piedra|marmol|m[áa]rmol",i][!"highway"][!"landuse"][!"place"][!"boundary"]',
]
...
# categoria manda craft; las demas solo entran si el elemento no la trae.
LLAVES_TIPO = ["craft", "shop", "amenity", "healthcare", "office",
