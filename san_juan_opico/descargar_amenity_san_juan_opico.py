...
# -*- coding: utf-8 -*-
"""
Descarga los servicios y equipamientos (amenity=* de OpenStreetMap) del
distrito de SAN JUAN OPICO, El Salvador.

Es la categoria mas amplia de OpenStreetMap. Incluye centros escolares,
colegios, universidades, hospitales y clinicas, farmacias, iglesias y
templos, alcaldias y juzgados, policia y bomberos, correos, bancos,
restaurantes, comedores y cafeterias, bares, gasolineras, parqueos,
mercados, funerarias y cementerios, bibliotecas, banos publicos, paradas de
bus, cajeros y el resto de los ~200 valores de amenity=*.
...
Uso:   pip install requests
       python3 descargar_amenity_san_juan_opico.py

Deja en la carpeta 'datos':
    servicios_san_juan_opico.csv
    servicios_san_juan_opico.geojson

Los CSV de todas las categorias y distritos tienen las mismas columnas, asi
que al final se pueden unir con:
    head -1 datos/servicios_san_juan_opico.csv >  datos/todos.csv
...
# Filtros de la categoria amenity=*. Primero se pide la llave completa, asi
...
FILTROS_POR_TAG = [
    'nwr["amenity"]',
    'nwr["office"~"^(government|educational_institution|religion|ngo|charity)$"]',
    'nwr["building"~"^(school|university|college|kindergarten|hospital|clinic|church|cathedral|chapel|mosque|temple|synagogue|civic|government|public|fire_station|train_station|transportation)$"]',
    'nwr["landuse"~"^(education|religious|cemetery|institutional)$"]["name"]',
]
...
FILTROS_POR_NOMBRE = [
    'nwr["name"~"escuela|colegio|instituto|universidad|kinder|parvular|centro escolar|complejo educativo|academia|hospital|cl[ií]nica|unidad de salud|casa de salud|iglesia|parroquia|catedral|templo|capilla|ministerio de|alcald[ií]a|juzgado|corte |polic[ií]a|pnc |bomberos|correos|banco |cajero|gasolinera|restaurante|comedor|pupuser|cafeter[íi]a|bar |parqueo|estacionamiento|mercado|funeraria|cementerio|biblioteca|museo|casa comunal|centro judicial|centro de gobierno|terminal|parada",i][!"highway"][!"landuse"][!"place"][!"boundary"]',
]
...
# categoria manda amenity; las demas solo entran si el elemento no la trae.
LLAVES_TIPO = ["amenity", "shop", "healthcare", "office", "craft",
