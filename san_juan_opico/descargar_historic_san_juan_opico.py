...
# -*- coding: utf-8 -*-
"""
Descarga el patrimonio historico (historic=* de OpenStreetMap) del distrito
de SAN JUAN OPICO, El Salvador.

Incluye monumentos y monolitos, estatuas y bustos, placas conmemorativas,
edificios historicos, casas de la cultura, museos, ruinas y sitios
arqueologicos, iglesias y cementerios historicos, fortificaciones, tumbas y
memoriales.
...
Uso:   pip install requests
       python3 descargar_historic_san_juan_opico.py

Deja en la carpeta 'datos':
    patrimonio_san_juan_opico.csv
    patrimonio_san_juan_opico.geojson

Los CSV de todas las categorias y distritos tienen las mismas columnas, asi
que al final se pueden unir con:
    head -1 datos/patrimonio_san_juan_opico.csv >  datos/todos.csv
...
# Filtros de la categoria historic=*. Primero se pide la llave completa, asi
...
FILTROS_POR_TAG = [
    ...
    'nwr["building"~"^(ruins|castle|cathedral)$"]',
]
...
FILTROS_POR_NOMBRE = [
    'nwr["name"~"monumento|monolito|estatua|busto|placa|memorial|plaza hist|casa de la cultura|museo|ruinas|sitio arqueol|patrimonio|hist[óo]ric|colonial|antigu[oa]|palacio|teatro nacional|catedral|cementerio general|fortaleza|cuartel|mausoleo",i][!"highway"][!"landuse"][!"place"][!"boundary"]',
]
...
# categoria manda historic; las demas solo entran si el elemento no la trae.
LLAVES_TIPO = ["historic", "shop", "amenity", "healthcare", "office",
               "craft", "industrial", "tourism", "leisure", "sport",
               "club", "man_made", "building", "landuse", "natural",
               "place", "cuisine", "brand"]
