...
# -*- coding: utf-8 -*-
"""
Descarga las instalaciones y actividades deportivas (sport=* de
OpenStreetMap) del distrito de SAN JUAN OPICO, El Salvador.

Incluye canchas de futbol, basquetbol, voleibol y tenis, estadios,
polideportivos, gimnasios y centros de acondicionamiento, piscinas, pistas
de atletismo, academias y escuelas deportivas, clubes deportivos, boliches,
gimnasios de artes marciales, skate parks y demas valores de sport=*.
...
Uso:   pip install requests
       python3 descargar_sport_san_juan_opico.py

Deja en la carpeta 'datos':
    deportes_san_juan_opico.csv
    deportes_san_juan_opico.geojson

Los CSV de todas las categorias y distritos tienen las mismas columnas, asi
que al final se pueden unir con:
    head -1 datos/deportes_san_juan_opico.csv >  datos/todos.csv
...
# Filtros de la categoria sport=*. Primero se pide la llave completa, asi
...
FILTROS_POR_TAG = [
    'nwr["sport"]',
    'nwr["leisure"~"^(pitch|sports_centre|stadium|track|swimming_pool|fitness_centre|fitness_station|golf_course|horse_riding|ice_rink|sports_hall|slipway|marina)$"]',
    'nwr["club"~"^(sport|football|athletics|swimming|martial_arts|cycling|chess)$"]',
    'nwr["building"~"^(stadium|sports_hall|sports_centre|grandstand|riding_hall)$"]',
    'nwr["shop"="sports"]',
]
...
FILTROS_POR_NOMBRE = [
    'nwr["name"~"cancha|estadio|polideportivo|gimnasio|gym|complejo deportivo|club deportivo|academia de|escuela de f[úu]tbol|piscina|pista|coliseo|arena|boliche|tenis|b[áa]squet|baloncesto|f[úu]tbol|futbol|b[ée]isbol|beisbol|softbol|voleibol|nataci[óo]n|atletismo|skate|crossfit|yoga|pilates|artes marciales|karate|taekwondo|judo|box|gimnasia|ciclismo|h[íi]pica",i][!"highway"][!"landuse"][!"place"][!"boundary"]',
]
...
# categoria manda sport; las demas solo entran si el elemento no la trae.
LLAVES_TIPO = ["sport", "shop", "amenity", "healthcare", "office",
               "craft", "industrial", "tourism", "leisure", "club",
