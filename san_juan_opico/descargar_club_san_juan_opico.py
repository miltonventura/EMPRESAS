...
# -*- coding: utf-8 -*-
"""
Descarga los clubes y organizaciones (club=* de OpenStreetMap) del distrito
de SAN JUAN OPICO, El Salvador.

Incluye clubes sociales y deportivos, asociaciones y fundaciones, ADESCO y
casas comunales, gremiales y camaras empresariales, sindicatos,
cooperativas, colegios profesionales, clubes de servicio (rotarios,
leones), grupos scouts, centros culturales y demas organizaciones sin fines
de lucro.
...
Uso:   pip install requests
       python3 descargar_club_san_juan_opico.py

Deja en la carpeta 'datos':
    clubes_san_juan_opico.csv
    clubes_san_juan_opico.geojson

Los CSV de todas las categorias y distritos tienen las mismas columnas, asi
que al final se pueden unir con:
    head -1 datos/clubes_san_juan_opico.csv >  datos/todos.csv
...
# Filtros de la categoria club=*. Primero se pide la llave completa, asi
...
FILTROS_POR_TAG = [
    'nwr["club"]',
    'nwr["office"~"^(association|ngo|political_party|union|charity|foundation|religion|guild)$"]',
    'nwr["amenity"~"^(social_facility|community_centre|social_centre|clubhouse)$"]',
    'nwr["leisure"~"^(club|social_club|sports_centre)$"]["name"]',
]
...
FILTROS_POR_NOMBRE = [
    'nwr["name"~"club|asociaci[óo]n|fundaci[óo]n|sociedad|gremio|gremial|sindicato|c[áa]mara |cooperativa|colegio de|logia|casa comunal|adesco|comit[ée]|hermandad|cofrad[íi]a|rotari|leones|scout|centro cultural|casa de la cultura|ong |organizaci[óo]n",i][!"highway"][!"landuse"][!"place"][!"boundary"]',
]
...
# categoria manda club; las demas solo entran si el elemento no la trae.
LLAVES_TIPO = ["club", "shop", "amenity", "healthcare", "office", "craft",
               "industrial", "tourism", "leisure", "sport", "historic",
