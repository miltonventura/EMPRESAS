...
# -*- coding: utf-8 -*-
"""
Descarga los lugares de recreacion (leisure=* de OpenStreetMap) del distrito
de SAN JUAN OPICO, El Salvador.

Incluye parques y plazas, canchas y complejos deportivos, estadios,
gimnasios, piscinas, turicentros y centros recreativos, clubes sociales,
salas de juego, boliches y billares, casinos, discotecas, teatros y cines,
jardines, miradores y areas verdes.
...
Uso:   pip install requests
       python3 descargar_leisure_san_juan_opico.py

Deja en la carpeta 'datos':
    recreacion_san_juan_opico.csv
    recreacion_san_juan_opico.geojson

Los CSV de todas las categorias y distritos tienen las mismas columnas, asi
que al final se pueden unir con:
    head -1 datos/recreacion_san_juan_opico.csv >  datos/todos.csv
...
# Filtros de la categoria leisure=*. Primero se pide la llave completa, asi
...
FILTROS_POR_TAG = [
    'nwr["leisure"]',
    'nwr["amenity"~"^(community_centre|social_centre|theatre|cinema|nightclub|casino|gambling|arts_centre|events_venue|gym|public_bath|swimming_pool|bbq|shelter)$"]',
    'nwr["sport"]',
    'nwr["landuse"~"^(recreation_ground|village_green|winter_sports)$"]',
    'nwr["building"~"^(stadium|sports_hall|sports_centre|grandstand|pavilion)$"]',
]
...
FILTROS_POR_NOMBRE = [
    'nwr["name"~"parque|plaza |cancha|gimnasio|gym|polideportivo|piscina|club |turicentro|centro recreativo|complejo deportivo|estadio|coliseo|boliche|billar|casino|discoteca|disco |teatro|cine|juegos|balneario|zool[óo]gico|jard[íi]n bot[áa]nico|mirador|[áa]rea verde|zona verde|cancha sint[ée]tica|sal[óo]n de eventos",i][!"highway"][!"place"][!"boundary"]',
]
...
# categoria manda leisure; las demas solo entran si el elemento no la trae.
LLAVES_TIPO = ["leisure", "shop", "amenity", "healthcare", "office",
               "craft", "industrial", "tourism", "sport", "club",
