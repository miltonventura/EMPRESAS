...
# -*- coding: utf-8 -*-
"""
Descarga los servicios de salud (healthcare=* de OpenStreetMap) del distrito
de SAN JUAN OPICO, El Salvador.

Incluye hospitales, clinicas y consultorios, unidades y casas de salud del
MINSAL, ISSS y FOSALUD, centros medicos, laboratorios clinicos, farmacias y
droguerias, opticas, clinicas dentales, especialistas (pediatria,
ginecologia, dermatologia, oftalmologia, cardiologia, ortopedia),
fisioterapia y rehabilitacion, psicologia y psiquiatria, nutricion,
veterinarias, ambulancias, asilos y hogares de ancianos.
...
Uso:   pip install requests
       python3 descargar_healthcare_san_juan_opico.py

Deja en la carpeta 'datos':
    salud_san_juan_opico.csv
    salud_san_juan_opico.geojson

Los CSV de todas las categorias y distritos tienen las mismas columnas, asi
que al final se pueden unir con:
    head -1 datos/salud_san_juan_opico.csv >  datos/todos.csv
...
# Filtros de la categoria healthcare=*. Primero se pide la llave completa, asi
...
FILTROS_POR_TAG = [
    'nwr["healthcare"]',
    'nwr["amenity"~"^(hospital|clinic|doctors|dentist|pharmacy|veterinary|nursing_home|social_facility|blood_donation|first_aid|baby_hatch)$"]',
    'nwr["shop"~"^(optician|hearing_aids|medical_supply|herbalist|nutrition_supplements|chemist)$"]',
    'nwr["building"~"^(hospital|clinic)$"]',
    'nwr["emergency"~"^(ambulance_station|defibrillator)$"]',
]
...
FILTROS_POR_NOMBRE = [
    'nwr["name"~"hospital|cl[íi]nica|clinica|unidad de salud|casa de salud|centro m[ée]dico|consultorio|policl[íi]nic|laboratorio cl[íi]nico|laboratorio|farmacia|droguer|[óo]ptica|dental|odontol|dentista|ortodon|pediatr|ginecol|dermatol|oftalmol|cardiol|ortoped|neurol|urolog|oncolog|radiolog|ultrasonograf|fisioterapia|rehabilitaci[óo]n|terapia|psicol|psiquiatr|nutrici|veterinar|ambulancia|cruz roja|comandos de salvamento|isss|fosalud|minsal|asilo|hogar de ancianos|banco de sangre",i][!"highway"][!"landuse"][!"place"][!"boundary"]',
]
...
# categoria manda healthcare; las demas solo entran si el elemento no la trae.
LLAVES_TIPO = ["healthcare", "shop", "amenity", "office", "craft",
